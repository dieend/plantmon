...
public class Ubi extends Plant{
    ...
    public Ubi(GridMap map, JPanel panel, Graphics2D g2d,int gx, int gy,int status) {
     ...
     @Override public void setPanen(){
        Game.instance().getInventory().add(new FoodItem(Item.BuUbi, null),1);
        super.setPanen();
    }
    public static void main(String[] str){
        JFrame frame=new JFrame();
        BufferedImage bf = new BufferedImage(1, 1, BufferedImage.TYPE_INT_ARGB);
        Ubi a = new Ubi(new GridMap(), new JPanel(), bf.createGraphics(), 0,0,1);
    ...
    public void setFase(int i) {
        ...
        if (fase == DEWASANOSIRAM){
            entity.load("picture/ubipanen.png");//, 1, 1, Utilities.GRIDSIZE, Utilities.GRIDSIZE);
        } else if (fase == REMAJANOSIRAM){
            entity.load("picture/ubiremaja.png");//, 1, 1, Utilities.GRIDSIZE, Utilities.GRIDSIZE);
        } else {
            super.setFase(i);
        }
    }
