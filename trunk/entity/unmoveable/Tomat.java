...
public class Tomat extends Plant{
    ...
    public Tomat(GridMap map, JPanel panel, Graphics2D g2d,int gx, int gy,int status) {
     ...
     @Override public JPanel get_Info(){
        ...
        image1=new ImageIcon(getClass().getResource("tomat.png"));
     ...
     @Override public void setPanen(){
        Game.instance().getInventory().add(new FoodItem(Item.BuTomat, null),1);
        super.setPanen();
    }
    public static void main(String[] str){
        JFrame frame=new JFrame();
        BufferedImage bf = new BufferedImage(1, 1, BufferedImage.TYPE_INT_ARGB);
        Tomat a = new Tomat(new GridMap(), new JPanel(), bf.createGraphics(), 0,0,1);
