...
public class Kentang extends Plant{
    ...
    public Kentang(GridMap map, JPanel panel, Graphics2D g2d,int gx, int gy,int status) {
     ...
     @Override public JPanel get_Info(){
        ...
        image1=new ImageIcon(getClass().getResource("kentang.png"));
     ...
     @Override public void setPanen(){
        Game.instance().getInventory().add(new FoodItem(Item.BuKentang, null),1);
        super.setPanen();
    }
    public static void main(String[] str){
        JFrame frame=new JFrame();
        BufferedImage bf = new BufferedImage(1, 1, BufferedImage.TYPE_INT_ARGB);
        Kentang a = new Kentang(new GridMap(), new JPanel(), bf.createGraphics(), 0,0,1);
    ...
    public void setFase(int i) {
        ...
        if (fase == DEWASANOSIRAM || fase == DEWASASIRAM){
            entity.load("picture/kentangpanen.png");//, 1, 1, Utilities.GRIDSIZE, Utilities.GRIDSIZE);
        } else if (fase == REMAJANOSIRAM || fase == REMAJASIRAM){
            entity.load("picture/kentangremaja.png");//, 1, 1, Utilities.GRIDSIZE, Utilities.GRIDSIZE);
        } else {
            super.setFase(i);
        }
    }
