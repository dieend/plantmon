...
public class Jagung extends Plant{
    ...
    public Jagung(GridMap map, JPanel panel, Graphics2D g2d,int gx, int gy,int status) {
     ...
     @Override public JPanel get_Info(){
        ...
        image1=new ImageIcon(getClass().getResource("jagung.png"));
     ...
     @Override public void setPanen(){
        Game.instance().getInventory().add(new FoodItem(Item.BuJagung, null),1);
        super.setPanen();
    }
    public static void main(String[] str){
        JFrame frame=new JFrame();
        BufferedImage bf = new BufferedImage(1, 1, BufferedImage.TYPE_INT_ARGB);
        Jagung a = new Jagung(new GridMap(), new JPanel(), bf.createGraphics(), 0,0,1);
    ...
    public void setFase(int i) {
        ...
        if (fase == DEWASANOSIRAM || fase == DEWASASIRAM){
            entity.load("picture/jagungpanen.png");//, 1, 1, Utilities.GRIDSIZE, Utilities.GRIDSIZE);
        } else if (fase == REMAJANOSIRAM || fase == REMAJASIRAM){
            entity.load("picture/jagungremaja.png");//, 1, 1, Utilities.GRIDSIZE, Utilities.GRIDSIZE);
        } else {
            super.setFase(i);
        }
    }
