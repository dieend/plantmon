...
public class Terong extends Plant{
    ...
    public Terong(GridMap map, JPanel panel, Graphics2D g2d,int gx, int gy,int status) {
     ...
     @Override public JPanel get_Info(){
        ...
        image1=new ImageIcon(getClass().getResource("terong.png"));
     ...
     @Override public void setPanen(){
        Game.instance().getInventory().add(new FoodItem(Item.BuTerong, null),1);
        super.setPanen();
    }
    public static void main(String[] str){
        JFrame frame=new JFrame();
        BufferedImage bf = new BufferedImage(1, 1, BufferedImage.TYPE_INT_ARGB);
        Terong a = new Terong(new GridMap(), new JPanel(), bf.createGraphics(), 0,0,1);
    ...
    public void setFase(int i) {
        ...
        if (fase == DEWASANOSIRAM || fase == DEWASASIRAM){
            entity.load("picture/terongpanen.png");//, 1, 1, Utilities.GRIDSIZE, Utilities.GRIDSIZE);
        } else if (fase == REMAJANOSIRAM || fase == REMAJASIRAM){
            entity.load("picture/terongremaja.png");//, 1, 1, Utilities.GRIDSIZE, Utilities.GRIDSIZE);
        } else {
            super.setFase(i);
        }
    }
