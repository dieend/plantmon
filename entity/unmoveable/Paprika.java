...
public class Paprika extends Plant{
    ...
    public Paprika(GridMap map, JPanel panel, Graphics2D g2d,int gx, int gy,int status) {
     ...
     @Override public JPanel get_Info(){
        ...
        image1=new ImageIcon(getClass().getResource("paprika.png"));
        image1.setImage(image1.getImage().getScaledInstance(100, 100, Image.SCALE_FAST));
        label1 = new JLabel(image1);
        label1.setBounds(0,0,100,100);
        panel.add(label1);
        label2 = new JLabel("Nama : Paprika");
     ...
     @Override public void setPanen(){
        Game.instance().getInventory().add(new FoodItem(Item.BuPaprika, null),1);
        super.setPanen();
    }
    public static void main(String[] str){
        JFrame frame=new JFrame();
        BufferedImage bf = new BufferedImage(1, 1, BufferedImage.TYPE_INT_ARGB);
        Paprika a = new Paprika(new GridMap(), new JPanel(), bf.createGraphics(), 0,0,1);
